...
var extensions = ['.xls', '.xlsx', '.ods', '.doc', '.docx', '.odt', '.rtf', '.ppt', '.pptx','.odp'];
  ...
  async.waterfall([
    function(cb) {
      if(path) {
        ...
        shellExec('loffice ' + options.join(' ') + ' --convert-to '+ 'pdf' + ':"writer_web_pdf_Export"' + ' --outdir /tmp ' + path, function(err, stdout, stderr) {
