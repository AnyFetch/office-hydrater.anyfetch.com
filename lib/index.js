  ...
  async.waterfall([
    function(cb) {
      if(!document.metadatas || !document.metadatas.path) {
        return cb(new hydrationError("Missing metadatas.path"));
      }
      else {
        var pdfFile = nativePath.basename(path) + '.pdf';

        var file = nativePath.basename(document.metadatas.path);
        ...
        if (!extensions[fileExtension]){
          return cb(new Error("Unknown file extension : " + fileExtension));
        }
