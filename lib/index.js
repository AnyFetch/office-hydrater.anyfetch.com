  ...
  async.waterfall([
    function(cb) {
      if(path) {
        ...
        console.log ('loffice ' + options.join(' ') + ' --convert-to html:"HTML (StarCalc)" --outdir /tmp ' + path);
        shellExec('loffice ' + options.join(' ') + ' --convert-to html:"HTML (StarCalc)" --outdir /tmp ' + path, function(err, stdout, stderr) {
