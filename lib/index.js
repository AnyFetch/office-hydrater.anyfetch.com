...
module.exports = function(path, document, changes, cb) {
  ...
  async.waterfall([
    ...
    function convertToPdf(converter, cb) {
      ...
      officeChild = shellExec('loffice ' + options.join(' ') + ' --convert-to pdf:"' + converter + '" --outdir /tmp ' + path, {maxBuffer: 20480 * 1024}, function(err, stdout, stderr) {
        officeChild = null;
...
process.on('exit', function() {
  if(officeChild) {
    console.log('Emergency SIGKILL of libreOffice');
    execSync('killall soffice.bin');
  }
});
