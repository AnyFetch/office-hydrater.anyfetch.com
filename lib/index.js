...
module.exports = function(path, document, changes, cb) {
  ...
  async.waterfall([
    ...
    function convertToPdf(converter, cb) {
      ...
      shellExec('loffice ' + options.join(' ') + ' --convert-to pdf:"' + converter + '" --outdir /tmp ' + path, {maxBuffer: 20480 * 1024, timeout: 55000, killSignal: 'SIGKILL'}, function(err, stdout, stderr) {
        // err.code to 139 means accessibility errors, we don't really care
        if(err && err.code !== 139) {
          console.log(require('util').inspect(err, true, 50));
