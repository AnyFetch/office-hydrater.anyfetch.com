...
module.exports = function(path, document, cb) {
  ...
  function(cb) {
    if(path) {
      ...
      var filesp = pathsp[pathsp.length-1].split('.'); filesp.pop();
      var outFile = filesp.join('.') + '.html';
      shellExec('loffice --headless --convert-to html --outdir /tmp/ ' + path, function(err) {
        cb(err, '/tmp/'+outFile);
      });
    }
  },
  ...
  function(outPath, data, cb) {
    ...
    document.binary_document_type = "office-html";
    cb(null, outPath);
  },
  function(outPath, cb) {
